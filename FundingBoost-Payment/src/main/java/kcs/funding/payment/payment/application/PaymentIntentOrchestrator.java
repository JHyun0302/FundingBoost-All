...
import static kcs.funding.payment.exception.ErrorCode.INTERNAL_SERVER_ERROR;
...
import kcs.funding.payment.payment.domain.PaymentAttemptStage;
...
public class PaymentIntentOrchestrator {
    ...
    @Transactional
    public boolean attachOrderIdIfAbsent(String intentKey, Long orderId) {
        PaymentIntent paymentIntent = paymentIntentStore.findByIntentKey(intentKey)
                .orElseThrow(() -> new CommonException(INTERNAL_SERVER_ERROR));
        boolean attached = paymentIntent.attachOrderIdIfAbsent(orderId);
        if (attached) {
            paymentIntentStore.update(paymentIntent);
        }
        return attached;
