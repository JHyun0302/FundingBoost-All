package kcs.funding.payment.payment.infra;
...
@ConditionalOnProperty(prefix = "app.payment", name = "schema-bootstrap-enabled", havingValue = "true", matchIfMissing = true)
public class PaymentSchemaInitializer {
    ...
    public void initializeSchema() {
        ...
        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS `%s`.`outbox_event` (
                    ...
                    KEY `idx_outbox_created_date` (`created_date`)
                ) ENGINE=InnoDB COMMENT='결제 서비스 로컬 아웃박스 이벤트 테이블'
                """.formatted(safeSchemaName));
    ...
    private void applyTableComments(String safeSchemaName) {
        ...
        String outboxEventTable = "`" + safeSchemaName + "`.`outbox_event`";
        ...
        jdbcTemplate.execute("""
                ALTER TABLE %s
                COMMENT = '결제 서비스 로컬 아웃박스 이벤트 테이블'
                """.formatted(outboxEventTable));
        jdbcTemplate.execute("""
                ALTER TABLE %s
                    MODIFY COLUMN `topic` VARCHAR(150) NOT NULL,
                    MODIFY COLUMN `event_key` VARCHAR(120) NOT NULL,
                    MODIFY COLUMN `event_type` VARCHAR(120) NOT NULL,
                    MODIFY COLUMN `aggregate_type` VARCHAR(80) NOT NULL,
                    MODIFY COLUMN `aggregate_id` VARCHAR(120) NOT NULL,
                    MODIFY COLUMN `payload` TEXT NOT NULL,
                    MODIFY COLUMN `headers` TEXT NULL,
                    MODIFY COLUMN `status` VARCHAR(20) NOT NULL,
                    MODIFY COLUMN `retry_count` INT NOT NULL,
                    ...
                    MODIFY COLUMN `created_date` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
                    MODIFY COLUMN `modified_date` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6)
                """.formatted(outboxEventTable));
    }
